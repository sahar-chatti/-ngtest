      ...
      return (
        <TableRow key={user.ID_UTILISATEUR}>
          ...
          {index === 0 && selectedOption === '0' && displayMode === 'card' && (
            <CardInvestisseurs displayMode={displayMode} searchTerm={searchTerm} selectedAvancement={selectedAvancement} setTotalObj={(tp, nn) => {
              let obj = { ...tot }
              obj.typeCli = tp
              obj.nbr = nn
              setTot(obj)
          ...
          {index === 0 && selectedOption === '2' && (
            ...
            <CardInvestisseursCSPD selectedClientType={"clientsCspd"} displayMode={displayMode} selectedTri={selectedTri} searchTerm={searchTerm} />
          )}
