...
const VisitorsManagement = () => {
    ...
    const handleOpen = () => {
        ...
        setFormData({
            ...
            prenom: '',
            departement: '',
            date_entre: new Date().toISOString().slice(0, 16), // Current date and time
            date_sortie: ''
        });
    };
                    ...
                    <TableBody>
                        {visitors.map((visitor) => (
                            <TableRow key={visitor.ID}>
                                ...
                                <TableCell>
                                    {new Date(visitor.DATE_ENTRE).toLocaleString()}
                                </TableCell>
                                <TableCell>
                                    {visitor.DATE_SORTIE ? new Date(visitor.DATE_SORTIE).toLocaleString() : '-'}
                                </TableCell>
                                <TableCell>
    ...
    required
    InputProps={{
        readOnly: !!editingVisitor // Only make it read-only if editing a visitor
    }}
/>
<TextField
    fullWidth
...
{!editingVisitor && (
    <TextField
        ...
        required
        InputProps={{
            readOnly: true // Always make the entry date read-only
        }}
    />
)}
...
<FormControlLabel
    control={
        <Checkbox
            checked={isExiting}
            onChange={(e) => setIsExiting(e.target.checked)}
            disabled={!editingVisitor} // Only allow checking if editing a visitor
        />
    }
