      ...
      {value === index && (
        <Box sx={{ p: 3 }}>
          ...
          {index === 0 && displayMode === 'card' && (
            <RHdemands selectedClientType={"clientsCspd"} setOpen={handleClickOpen} displayMode={displayMode} searchTerm={searchTerm} />
          )}
    ...
    <Box sx={{ width: '100%' }}>
      <Box sx={{ borderBottom: 1, borderColor: 'divider' }}>
        <Tabs value={value} onChange={(e, newValue) => handleChange(newValue)} aria-label="basic tabs example">
          ...
          <Tab label="Réclamations maintenance" {...a11yProps(0)} />
        </Tabs>
      </Box>
