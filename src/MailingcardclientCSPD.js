...
import emailm from './icons/gmail.png'
...
function CustomCard({ client,selectedClientType,user,setSelectedCli }) {
...
// alert(JSON.stringify(client))
 return await axios.post('http://192.168.1.170:3200/signin', { email:client.EMAIL,loginmail, username,code }, {
        ...
        .catch(error => {
            alert(error)
            setMessages('Error sending the informations');
        });
}
  ...
  return (
    ...
    <CustomCardWrapper style={{ textAlign: 'center' }}>
      <CustomCardContent>
      <GlowingBox  style={{ backgroundColor:client.CC_BLOQUER?"red":"green"}}>
      <Typography
  variant="h6"
  component="div"
  align="center"
...
>
  {client.INTITULE_CLIENT} <span style={{ fontSize: '0.8rem', fontWeight: 'normal' }}>({client.INTITULE_GR})</span>
        ...
        <Typography variant="h6" component="div" gutterBottom style={{ display: "flex", alignItems: "center", marginBottom: 10 }}>
          <img src={emailm} alt="person icon" style={{ marginRight: 8, width: "20px", height: "20px" }} />
...
const CardContainer = ({ searchTerm,selectedClientType,selectedTri ,selectedAll}) => {
  ...
  useEffect(() => {
    const selectedEmails = clients
      .filter(client => client.selectedCli)
      .map(client => client.EMAIL)
      .join(';');
    setBoite(selectedEmails);
  }, [clients]);
  ...
  const fetchPart = async () => {
    ...
    try {
      ...
      //copy array with the fieled related to the select checkbox 
      const response = await axios.get(URL, { params });
      let clientsList=[...response.data.clients]
      let FinalList=[]
      console.log("listClientcspd",FinalList)
      clientsList.map((row,i)=>{
      FinalList.push({...row,selectedCli:false,EMAIL:i==0?'gerant@click.com.tn':i==1?'amer_triki@yahoo.fr':'sahar.chatti@isimg.tn'})
  ...
  <div style={{ overflowY: 'auto', maxHeight: 'auto' }}>
    ...
    <Box sx={{ display: 'flex', flexWrap: 'wrap', justifyContent: 'space-between' }}>
      {clients.map((client,i) => (
        ...
        client={client} selectedClientType={selectedClientType} user={user}  setSelectedCli={(setSelectedCli)=>{
