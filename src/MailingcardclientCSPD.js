        ...
        <GlowingBox style={{ backgroundColor: client.CC_BLOQUER ? "red" : "#7695FF", borderRadius: '11px' }}>
          ...
          {!client.CC_BLOQUER && (
            <Checkbox
              ...
              {...label}
              checked={client.selectedCli}
              onChange={(e) => setSelectedCli(e.target.checked)}
            />
          )}
          {client.CC_BLOQUER && (
...
const CardContainer = ({ searchTerm, selectedClientType, setBoite, selectedTri, selectedAll }) => {
  ...
  useEffect(() => {
    ...
    setBoite(listMail)
    console.log('boite email', listMail)
  ...
  useEffect(() => {
    ...
    console.log('boite emails', selectedAll)
  ...
  const fetchPart = async () => {
    ...
    try {
      ...
      let finalList = [];
      console.log("listClientcs", clientsList)
      ...
      setLoading(false);
      console.log("listClientcs2", finalList)
