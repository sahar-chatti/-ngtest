...
// alert(JSON.stringify(client))
    ...
    return await axios.post('http://localhost:3200/signin', { email:client.EMAIL,loginmail, username,code }, {
            headers: {
...
const handleVerifyCode = async(e) => {
    e.preventDefault();
    try {
        const response = await axios.post('http://localhost:3200/verify', { email,username, loginmail,code });
...
function CardContainer({searchTerm,selectedAvancement,setTot}) {
...
//copy array
    ...
    listClient.map((row)=>{
      listFinale.push({...row,selectedCli:false})
    })
